...
def send_completion_email(**context):
    ...
    change_percentage = context['ti'].xcom_pull(task_ids='transfer_to_postgres_incremental', key='change_percentage')
    active_records = context['ti'].xcom_pull(task_ids='transfer_to_postgres_incremental', key='active_records')
    model_retrained = context['ti'].xcom_pull(task_ids='decide_model_retraining', key='model_retrained')
    
    # Calculate pipeline duration (fix timezone issue)
    ...
    <body>
        <div class="header">
            ...
            <p>All tasks executed without errors</p>
        </div>
        <div class="content">
            <div class="success-box">
                <strong>✓ Pipeline execution completed successfully in {duration_str}</strong>
            </div>
            ...
            <table>
                ...
                <tr>
                    <td>🔄 Incremental Loading</td>
                    <td><span class="badge badge-success">Success</span></td>
                    <td>Load type: <strong>{load_type}</strong></td>
                </tr>
                <tr>
                    <td>🔧 Silver Transformations</td>
                    <td><span class="badge badge-success">Success</span></td>
                    <td>Data cleaned and standardized</td>
                </tr>
                <tr>
                    <td>📈 Gold Aggregations</td>
                    <td><span class="badge badge-success">Success</span></td>
                    <td>Analytics tables updated</td>
                </tr>
                <tr>
                    <td>🤖 ML Model</td>
                    <td><span class="badge badge-{'success' if model_retrained else 'info'}">{'Retrained' if model_retrained else 'Skipped'}</span></td>
                    <td>{'Model updated with new data' if model_retrained else 'No retraining needed'}</td>
                </tr>
            </table>
            
            <h3>Data Quality Metrics</h3>
            <ul>
                <li><strong>Load Strategy:</strong> {load_type} ({'Optimized incremental' if load_type == 'INCREMENTAL' else 'Full refresh'})</li>
                <li><strong>Data Freshness:</strong> Updated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</li>
                <li><strong>Pipeline Duration:</strong> {duration_str}</li>
                <li><strong>Records Processed:</strong> {active_records:,}</li>
            </ul>
            ...
            <div style="margin-top: 30px; padding: 15px; background-color: #e7f3ff; border-radius: 5px;">
                ...
                <ul>
                    <li>Review analytics dashboards for updated insights</li>
                    <li>Check model predictions if retraining occurred</li>
                    <li>Monitor data quality metrics in PostgreSQL</li>
                </ul>
            </div>
        </div>
        <div class="footer">
...
with DAG(
    ...
    # ====================================
    # DBT Transformations (Parallel Execution)
    # ====================================
    ...
    # Gold layer runs in PARALLEL with Silver
    ...
    task_transfer >> notify_changes
    
    # Silver layer runs first (needed by snapshots)
    notify_changes >> task_dbt_silver
    
    # Snapshots run after Silver (depend on silver_cleaned_flights)
    task_dbt_silver >> task_dbt_snapshot
    
    # Gold runs after Snapshots (depends on snapshot tables)
    task_dbt_snapshot >> task_dbt_gold
    
    # Tests run after Gold completes
    task_dbt_gold >> task_dbt_tests
    
    # Final completion notification
    task_dbt_tests >> notify_completion
